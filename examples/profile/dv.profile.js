...
function dv_profile_cache(evt, query) {
    ...
    var dims = query.dims, idx = [], i, dat,
        keys = dims.map(function(d, i) {
            idx.push(i);
            return d/*.key*/;
        });

    idx.sort(cmp);
    var key = idx.map(function(j) {
        return keys[j];
    }).join("__");

    if (!(dat = evt.cache[key])) {
        // cache miss: execute the query
        dims = idx.map(function(j) {
            return dims[j];
        });
        window.dat = {
            evt: evt,
...
dv.profile = function(data) {
    ...
    g.benchmark = function(callback) {
        ...
        function printstats() {
            ...
            var sf = bd.frames.reduce(function(a, b) {
                return a + b * b;
            }, 0);
            var ss = bd.select.reduce(function(a, b) {
                return a + b * b;
            }, 0);
            var sr = bd.render.reduce(function(a, b) {
                return a + b * b;
            }, 0);
